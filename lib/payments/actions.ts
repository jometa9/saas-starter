...
const DEFAULT_PRICE_ID =
  process.env.STRIPE_DEFAULT_PRICE_ID || PREMIUM_MONTHLY_PRICE_ID;
...
export async function directCheckoutAction(): Promise<{
  error?: string;
  redirect?: string;
}> {
  try {
...
export async function customerPortalAction(): Promise<{
  error?: string;
  redirect?: string;
}> {
  try {
