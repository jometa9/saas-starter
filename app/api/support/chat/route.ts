...
import { sendMessageToAssistant, getUserThreadInfo, clearUserThread } from "@/lib/openai/assistant";
...
export async function POST(req: NextRequest) {
  try {
    ...
    try {
      const response = await sendMessageToAssistant(currentUser.id, message.trim());
      ...
      if (assistantError instanceof Error) {
        ...
        if (assistantError.message.includes('assistant')) {
          return NextResponse.json({
            error: 'The assistant is not available at this time. Please contact support.',
            fallback: true
          }, { status: 503 });
        }
      }
...
export async function GET(req: NextRequest) {
  try {
    ...
    return NextResponse.json({ error: 'Error getting thread info' }, { status: 500 });
  }
} 