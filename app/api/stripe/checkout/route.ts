...
export async function GET(req: NextRequest) {
  ...
  if (!sessionId) {
    return NextResponse.redirect(
      new URL("/dashboard?error=missing-session", req.url)
    );
  }
  ...
  try {
    ...
    if (!session) {
      return NextResponse.redirect(
        new URL("/dashboard?error=invalid-session", req.url)
      );
    }
    ...
    if (!session.customer) {
      return NextResponse.redirect(
        new URL("/dashboard?error=missing-customer", req.url)
      );
    }

    if (!session.subscription) {
      return NextResponse.redirect(
        new URL("/dashboard?error=missing-subscription", req.url)
      );
    }
    ...
    if (!subscriptionId || !customerId) {
      return NextResponse.redirect(
        new URL("/dashboard?error=incomplete-data", req.url)
      );
    }
    ...
    if (!item || !item.price) {
      return NextResponse.redirect(
        new URL("/dashboard?error=missing-price-data", req.url)
      );
    }
    ...
    if (!user) {
      return NextResponse.redirect(
        new URL("/sign-in?error=no-session", req.url)
      );
    }
    ...
    try {
      ...
      return NextResponse.redirect(
        new URL("/dashboard?error=update-error", req.url)
      );
    }

    // Redirigir al dashboard con mensaje de éxito
    return NextResponse.redirect(
      new URL("/dashboard?success=subscription-activated", req.url)
    );
  } catch (error) {
    if (error instanceof Error) {
    }
    return NextResponse.redirect(
      new URL("/dashboard?error=checkout-error", req.url)
    );
  }
}
