...
export async function GET(req: NextRequest) {
  ...
  if (process.env.NODE_ENV === 'production') {
    return NextResponse.json({ error: 'Este endpoint solo está disponible en modo de desarrollo' }, { status: 403 });
  }
  ...
  try {
    ...
    if (!user) {
      return NextResponse.json({ error: 'No hay sesión de usuario' }, { status: 401 });
    }
    ...
    try {
      ...
      pricesError = error instanceof Error ? error.message : 'Error desconocido';
    }
    ...
    try {
      if (user.stripeCustomerId && pricesData && pricesData.length > 0) {
        ...
        sessionTestResult = "No se pudo probar la creación de sesión (falta customerId o precios)";
      }
    } catch (error) {
      sessionTestError = error instanceof Error ? error.message : 'Error desconocido';
    }
    ...
    if (user.stripeCustomerId) {
      try {
        ...
        customerError = error instanceof Error ? error.message : 'Error desconocido';
      }
    }
    ...
    console.error('Error en diagnóstico de pago:', error);
