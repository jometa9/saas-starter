...
export default function PricingComponent({ user }: PricingComponentProps) {
  ...
  useEffect(() => {
    ...
    if (!user) {
      router.push(
        `/sign-in?redirect=${encodeURIComponent("/dashboard/pricing")}`
      );
      return;
    }
  ...
  const handleCheckout = async (plan: string, priceId: string | undefined) => {
    try {
      ...
      // Check if it's a redirect response (for Stripe checkout)
