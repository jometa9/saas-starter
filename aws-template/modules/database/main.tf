...
resource "aws_db_instance" "postgres_instance" {
  identifier             = "${var.environment_name}-postgres-instance"
  engine                 = "postgres"
  ...
  tags = {
    Name = "${var.environment_name}-postgres-instance"
  }
}


# Create a DB parameter group for PostgreSQL
resource "aws_db_parameter_group" "postgres_parameter_group" {
  name        = "${var.environment_name}-postgres-pg"
  family      = "postgres16"
